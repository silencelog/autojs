...
function ant_forest_enter (time_interval, retry_times) {
  ...
  /*
     ...
     *         1 失败
     */
  if (time_interval < 1000 || time_interval > 5000) {
    return RET_ERR_INVALID_TIME_INTERVAL
  }

  if (retry_times < 1 || retry_times > 20) {
...
function collect_energy (max_energy_num) {
  var index = 0
  if (max_energy_num < 0 || max_energy_num > 50) {
...
function go_friend_homepage_steal (max_friend_num) {
  ...
  if (max_friend_num < 0 || max_friend_num > 1000) {
  ...
  while (1) {
    ...
    /* 滑动 */
    gesture(random(300, 600), [random(50, device.width - 50), random(device.height * 7 / 9, device.height * 7 / 9 - 100)], [random(50, device.width - 50), random(device.height * 2 / 9, device.height * 2 / 9 - 100)]
